...
class Robot:
    ...
    def toward_free(self, dest):
        for p in sorted(self.legit_around(self.location), key=lambda x: wdist(x, dest)):
            if self.occ(p) == 'free':
                return p
    ...
    def freest(self, loc):
        return sorted([loc for loc in locs_around(loc) if self.occ(loc) == 'free'], key=lambda x: self.free_around(x), reverse=True)
    ...
    def act(self, game):
        ...
        self.turn = game['turn']
        to_c = self.toward2(my_loc, CENTER_POINT)
        ...
        if 'spawn' in loc_types(my_loc):
            if game['turn'] % 10 == 0:
                print "{} spawn turn, moving to safe square {}".format(my_loc, to_c)
                if not free_sqs:
                    return ['suicide']
            ...
            if game['turn'] % 10 == 9 and to_c in robots:
                print "{} moving to safe square {}".format(my_loc, self.safest(my_loc)[0])
                return ['move', self.safest(my_loc)[0]]
